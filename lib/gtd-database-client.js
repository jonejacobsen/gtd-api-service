...
class GTDDatabaseClient {
  ...
  async connect() {
    ...
    // Use Railway PostgreSQL URL or local connection
    const connectionString = process.env.RAILWAY_POSTGRESQL_URL || 
                           process.env.DATABASE_URL ||
                           process.env.PRODUCTIVITY_DB_URL;

    if (!connectionString) {
      throw new Error('No database connection string found. Please set RAILWAY_POSTGRESQL_URL or DATABASE_URL');
    }
